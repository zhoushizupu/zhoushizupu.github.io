...
for person_id, person in persons.items():
    # 提取现有数据
    birth_place = person.get('birthPlace', '广西钦州市灵山县那隆镇长福石岭垌村')
    # 如果没有 birthPlace，使用 occupation 或其他信息填充
...
# 保存新数据
with open('data/family.json', 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print('转换完成！')
print(f'转换人数：{len(converted_persons)}')
print('文件已更新为：data/family.json')
